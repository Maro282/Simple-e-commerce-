...
categoryFilter.addEventListener( "change", ( e ) => {
    ...
    if ( selectedCategory[0] ) {
        ...
        filteredProducts = productsData.filter(product => {
            return selectedCategory.includes(product.category) || selectedCategory.includes("wedding") && product.forWedding;
        } )
        generateProducts()
...
function generateProducts () {
    productEle = filteredProducts.map( product => {
        return (
    ...
    document.querySelector(".products-box .row").insertAdjacentHTML("beforeend", productEle);
}
